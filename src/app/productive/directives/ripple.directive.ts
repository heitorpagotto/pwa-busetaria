...
export class RippleDirective implements AfterViewInit {
   ...
   */
  @Input('pRippleCentered') centered: boolean = false;
  /**
   ...
   */
  @Input('pRippleUnbounded') unbounded: boolean = false;
   ...
   */
  @Input('pRippleRadius') radius: number = 0;
  /**
  ...
  public ngAfterViewInit(): void {
    ...
    if (!this.disabled) {
      if (this.pRippleTriggerFor) {
        const element: HTMLElement = this.pRippleTriggerFor;
  ...
  public get shouldBeForAnotherElement(): boolean {
    return !this.pRippleTriggerFor;
  }

  @HostBinding('class.p-ripple-unbounded')
  public get unbound(): boolean {
  ...
  // Receives an HTMLElement being the element at which to create the effect
  // Receives an X coordinate, being the clientX
  // Receives an Y coordinate, being the clientY
  ...
  // Calculates the distance to the furthest corner of the element
